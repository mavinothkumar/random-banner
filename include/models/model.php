...
function bc_rb_delete_by_id( $id ) {
	global $wpdb;
	$table  = $wpdb->prefix . BC_RB_RANDOM_BANNER_DB;
	$verify = $wpdb->delete( $table, array( 'id' => (int) $id ), array( '%d' ) );
	if ( $verify ) {
		return 'success';
	}
...
function search_for_category_exist( $table, $post ) {
	...
	$success  = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM $table WHERE category LIKE '%s'", bc_rb_sanitize_text_field( $category ) ) );
	if ( ! is_null( $success ) ) {
		return true;
	} else {
		return false;
...
function bc_rb_update_random_banner_category_by_delete( $category ) {
	...
	$wpdb->update(
		...
		array( 'category' => bc_rb_sanitize_text_field( strtolower( $category ) ) ),
		array( '%s' ),
		array( '%s' )
	);
...
function bc_rb_update_random_banner_category_by_update( $new_category, $old_category ) {
	...
	$wpdb->update(
		$table,
		array(
			'category' => bc_rb_sanitize_text_field( strtolower( $new_category ) ),
		),
		array( 'category' => bc_rb_sanitize_text_field( strtolower( $old_category ) ) ),
		array( '%s' ),
		array( '%s' )
	);
}
 ...
 * @param string $category Category name.
 *
 * @return array Banner by Category Name
 */
